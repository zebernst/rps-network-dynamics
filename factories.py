...
class ClusteredGraphBuilder:
    ...
    def with_graph_size(self, size: int) -> "ClusteredGraphBuilder":
        self.num_nodes = size
        return self
    ...
    def with_edge_degree_distribution(
        ...
        if distribution == zipf:
            if 'a' not in kwargs:
                raise nx.NetworkXUnfeasible("zipf distribution requires an 'a' parameter to be set.")
        elif distribution == uniform:
            if 'high' not in kwargs or 'low' not in kwargs:
                raise nx.NetworkXUnfeasible("uniform distribution requires both a 'high' "
                                            "and a 'low' parameter to be set.")
        elif distribution == constant:
            if 'k' not in kwargs:
                raise nx.NetworkXUnfeasible("constant distribution requires a 'k' parameter to be set.")
            elif (kwargs["k"] * self.num_nodes) % 2 != 0:
            ...
            raise nx.NetworkXUnfeasible("invalid distribution.")
